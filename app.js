...
function getGroceryList(plan) {
  ...
  function getCanonicalKeyword(keyword) {
    return GROCERY_KEYWORD_ALIASES[keyword] ?? keyword;
  }

  Object.entries(GROCERY_CATEGORIES).forEach(([category, keywords]) => {
    keywords.forEach((keyword) => {
      const groceryItem = getCanonicalKeyword(keyword);
      if (hasKeywordMatch(combinedText, keyword) && !groceries[category].includes(groceryItem)) {
        groceries[category].push(groceryItem);
      }
    });
  });
